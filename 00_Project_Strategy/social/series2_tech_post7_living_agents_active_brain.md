...
In SOLL, with the Living Patient switched on, care is pushed. Across the same 100 runs, the same town generates about 324 proactive alerts per run (0 to 324, Cohen's d of about 4.1), and about 173% more chronic-care (ketenzorg) interventions (69 to 188 per run, d of about 1.5, significant). Administrative waste falls from roughly €33,600 to roughly €5,600 per run as the ambient AI absorbs the paperwork that was consuming about 30% of clinician time. The active brain does not invent demand. It re-times it, moving care upstream of the crisis.

Those alerts are not just counters; they are outward actions, and I now log them as such. Every proactive alert is also a GP pre-brief sent over A2A, so the clinician arrives already informed. About 17 distinct citizens are reached per run, and the reach is spread across every age band (roughly 97, 72, 86, and 68 contacts per run for the 0-44, 45-64, 65-79, and 80-plus groups), which is the raw material the fairness monitor watches. Consent is not decorative either: about 61 would-be outreach attempts per run land on a citizen who has opted out and are dropped, never contacted. The right to be left alone survives contact with a system whose default is to act.
...
- *Caption:* 100 runs of 3,000 cycles. Switching on the Living Patient (SOLL) takes proactive interventions from 0 to about 324 per run and chronic-care (ketenzorg) interventions up about 173% (69 to 188, d of about 1.5, significant), while administrative waste falls from about €33.6k to about €5.6k per run. Emergency-room load does not change significantly (67 to 64), so prevention re-times demand upstream rather than visibly adding or removing acute load at this town size.
- *Alt text:* A data table comparing IST and SOLL modes, showing proactive alerts rising from zero to about three hundred and twenty-four per run, chronic-care interventions up about 173%, and administrative waste falling by roughly 83% under SOLL.
